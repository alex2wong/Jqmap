...
function init() {
    ...
    geourl = "http://58.198.183.213:8099/";
    geourl2 = "http://58.198.183.239:8080/";    //一个子节点geoserver202地址
    //geourl3 = "http://172.29.11.24:8000/";        //第二个子节点 geoserver 202地址
    //Tileurl= "http://58.198.183.160:86/";
    ...
    var baseMap = new OpenLayers.Layer.WMS("世界是平的", geourl + "geoserver/wms",
        {
            layers: 'cite:city',
            srs: 'EPSG:4326',
            format: 'image/png'
    ...
    var WHland = new OpenLayers.Layer.WMS("武汉", geourl + "geoserver/wms", {
        layers: 'cite:landsatWHB5',
        srs: "EPSG:32649",
        format: "image/jpeg",
        transparent: true
...
//                labelOutlineWidth: 
    ...
    function overlay_getTileURL(bounds) {  //this 指的是tms 这个图层
        ...
        if (this.maxExtent.intersectsBounds(bounds)) {
          ...
          return Tileurl + z + "/" + x + "/" + y + "." + this.type;
        } else {
            return "";
        }

    }
//    var tianditu = new OpenLayers.Layer.XYZ("天地图",
...
//        );
//    var tiandituAnno =new OpenLayers.Layer.XYZ(
//        "天地图中文注记",
//        [
...
//                "http://webrd04.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7"]
   ...
   // map.addLayer(SHlandsat); 
    ...
    map.addControl(new OpenLayers.Control.OverviewMap(option));
...
//geolocate模块
    ...
    vector = new OpenLayers.Layer.Vector('vector');  //标注定位点的矢量图层
    map.addLayer(vector);
...
//    //start of MeasureControls
//    measureControls = {  //line ,polygon is Key. measureControls is a dic
...
//        control.events.on({
//            "measure": handleMeasurements,  //为两个控件注册两个事件
...
//    var out = "";
//    if (order == 1) {
//        out += "  测量结果:" + measure.toFixed(3) + " " + units;   //order==1 输出：measure +  event.measure + KM ，测量长度
//    } else {
//        out += "  测量结果:" + measure.toFixed(3) + " " + units + "<sup>2</" + "sup>"; // KM2 面积单位
//    }
...
function searchplace() {
    ...
    function (json) { shiplayer.removeAllFeatures(); DrawPoint(json); $("#list").listview('refresh'); }, "json")
...
function searchnear() {
    ...
    $.ajax({
        ...
        data: param,
        dataType: "json",
        success: function (json) { shiplayer.removeAllFeatures(); DrawCircle(X, Y, 2000); DrawPoint(json); $("#list").listview('refresh'); },
